...
public class CombatAnimationController : MonoBehaviour
{
    ...
    {
        Debug.Log($"▶ HandlePlayerSkill: {data.displayName}");
        switch (data.displayName)  // 또는 data.id, data.skillName 등에 맞게
        {
            case "물기":
                playerAnimator.SetTrigger("Attack");
                StartCoroutine(DoAttackStep(playerCharacter.transform));
                break;
            case "으르렁거리기":
    ...
    private IEnumerator DoAttackStep(Transform tf)
    {
